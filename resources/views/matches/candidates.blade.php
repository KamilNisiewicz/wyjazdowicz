                ...
                <div class="max-w-xl">
                    <header>
                        <h2 class="text-lg font-medium text-gray-900">
                            {{ __('Potwierdź miejscowość') }}
                        </h2>

                        <p class="mt-1 text-sm text-gray-600">
                            {{ __('Wybierz właściwą miejscowość z listy znalezionych przez wyszukiwarkę OpenStreetMap.') }}
                        </p>
                    </header>
                    ...
                    <form method="post" action="{{ route('matches.store') }}" class="mt-6 space-y-6">
                        ...
                        <input type="hidden" name="goals_for" value="{{ $goals_for }}">
                        <input type="hidden" name="goals_against" value="{{ $goals_against }}">
                        ...
                        <div class="flex items-center gap-4">
                            <x-primary-button>{{ __('Zapisz mecz') }}</x-primary-button>
                            <a href="{{ route('matches.create') }}" class="text-sm text-gray-600 underline">{{ __('Wróć i popraw dane') }}</a>
                        </div>
                    </form>
                </div>
            </div>
