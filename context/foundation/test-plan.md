...
| 4 | A Tailwind/Vite frontend build silently succeeds (exit 0) without actually compiling a newly-used utility class, so the UI looks broken in production, with nothing in CI catching it | Medium | Medium | Interview Q2; `archive/2026-07-26-edit-delete-match/plan.md` (this already happened once, in a local build — Phase 4 research confirmed CI's `node-version: lts/*` plus gitignored `public/build/` already rules out the Node-version mechanism in production; the open gap is the missing deterministic check on build *output*, not build environment) |
...
| #4 | After adding a new Tailwind class to any Blade view and running `npm run build` in CI, the compiled CSS actually contains that class — verified by a deterministic post-build check, not assumed from a zero exit code | "The Blade code is correct, so the style will work" — this exact assumption broke in S-04 (a local build under the wrong Node version, not a CI build); also challenge "CI already pins `node-version: lts/*`, so this risk is closed" — that rules out the Node-version *mechanism* but not a build that silently omits a class for any other reason (config regression, dependency upgrade) | Confirmed by Phase 4 research: `.github/workflows/deploy.yml` already pins `node-version: lts/*` and `public/build/` is gitignored, so CI never ships a stale/wrong-Node local build; the open gap is a missing content check on the build's own output | Quality gate / deterministic check (grep compiled CSS for a known set of used classes), not a classic unit test | Re-testing the already-closed Node-version angle instead of the actual open gap (build-output verification); or relying on `export PATH=...v20...` by hand for *local* builds as if it were a CI mitigation |
...
| 4 | Quality-gates wiring | Require the test suite to pass before deploy; catch stale frontend builds | #1, #4 | gates (CI) | complete | `context/changes/testing-quality-gates-wiring/` |
...
| CI test gate | GitHub Actions | n/a | Wired §3 Phase 4 — `test` job (`php artisan test`) gates the `deploy` job via `needs:`; compiled-CSS class check gates the asset-upload step |
...
| unit + integration in CI | CI on push to `master` (`test` job, `needs:` gates `deploy`) | required (wired §3 Phase 4) | logic regressions reaching production undetected |
| frontend build verification (deterministic compiled-CSS class check) | CI (`deploy` job, before asset upload) | required (wired §3 Phase 4) | silently incomplete builds (Risk #4) |
...
- **Location**: `.github/workflows/deploy.yml` — the project's only CI/CD workflow.
- **Pattern: required-before-deploy gate**. Add a new top-level job (reference: the `test` job) and make `deploy` depend on it via `needs: <job-name>` on the `deploy:` job definition. This is the only mechanism that actually blocks the pipeline — wiring a check as a *step* inside the `deploy` job itself does not stop anything upstream of it in a meaningful gate sense once other jobs exist; `needs:` is what GitHub Actions actually honors to skip a dependent job when its dependency fails.
- **Reference test job**: `test` runs `php artisan test` (PHP 8.5, matching production) after `npm run build` — the build step is required even though the job doesn't deploy anything, because feature tests that render Blade views through `@vite()` need `public/build/manifest.json` to exist, or they fail with "Vite manifest not found" (discovered live during §3 Phase 4 implementation — the manifest is not optional for any test that hits a route rendering a full page).
- **Pattern: deterministic build-output check**. Reference: the "Verify compiled CSS contains known responsive classes" step in the `deploy` job, positioned after `npm run build` and before the asset-upload step. Locates the compiled CSS under `public/build/assets/*.css`, then `grep -qF` (fixed-string match, not regex) each class in a small hand-maintained list against the file. Extend the list by adding one entry when a new class is introduced whose loss would be easy to miss visually (e.g. a new responsive breakpoint class).
- **Gotcha — escaped colons**: Tailwind compiles `sm:hidden` to the CSS selector `.sm\:hidden{...}` — the colon is escaped with a literal backslash in the compiled output. A grep pattern for the bare class name (`sm:hidden`, no backslash) never matches and reports every class missing on every build, including correct ones. Always grep for the class with `:` replaced by `\:`, and use `grep -F` (fixed-string) rather than a bare regex so the backslash is matched literally instead of being interpreted.
- **Gotcha — vendor-view contamination**: Tailwind's `content` glob in this project includes Laravel's own vendor pagination template (`./vendor/laravel/framework/src/Illuminate/Pagination/resources/views/*.blade.php`), which itself uses `sm:hidden`, `sm:flex`, `sm:flex-1`, `sm:gap-2`, `sm:items-center`, and `sm:justify-between`. Any class also used by that vendor template will *always* compile into the CSS regardless of whether the app's own views still use it — checking for it proves nothing. Before adding a class to the check list, confirm it does **not** appear in `vendor/laravel/framework/src/Illuminate/Pagination/resources/views/tailwind.blade.php` (discovered live during §3 Phase 4: `sm:hidden` and `sm:flex` were dropped from the seed list for exactly this reason, even though `sm:hidden` was the actual class that broke in the S-04 incident — `sm:block`, used in the same view for the same responsive split, still gives direct signal for that regression).
- **Verification approach**: local verification (strip every source file that declares the checked classes, clear Laravel's compiled view cache with `php artisan view:clear` — a stale cache under `storage/framework/views/*.php` is also in the Tailwind content glob and will mask a source-level removal — rebuild, confirm the grep reports the classes missing; restore, rebuild, confirm it reports them present) is an acceptable substitute for a live branch push when the check logic is pure, environment-independent bash (no network calls, no CI-specific behavior). A live-branch push is still the right call for a `needs:`-based job gate, since GitHub Actions' cross-job scheduling behavior is what's actually under test there, not local logic — but budget for it carefully: this project's `deploy` job completes in ~30s, faster than a human-paced poll-and-cancel loop can reliably interrupt before the rsync/SSH steps run. A stripped-content-only push (job fails at the check step, never reaches rsync) is zero-risk; a restored-content push lets `deploy` run to completion and will perform a real (if content-identical and idempotent) deploy to production — decide in advance whether that's acceptable or whether the verification branch temporarily needs an `if: github.ref == 'refs/heads/master'` guard on the upload/SSH steps.
- Checked: 2026-07-31 (rollout §3 Phase 4).
